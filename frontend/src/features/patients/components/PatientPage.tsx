...
import { UserFiles } from "@/features/patient-files";
import { VisitTable } from "@/features/visits";
import { isDoctorRole } from "@/shared";
import { usePatientRegistry } from "../hooks";
...
const PatientPage: FC<PatientPageProps> = ({ patientId }) => {
  const { id } = useParams();
  ...
  const { isLoading, error } = patientRegistryQuery;
  const data = patientRegistryQuery.data as PatientRegistryRes;
