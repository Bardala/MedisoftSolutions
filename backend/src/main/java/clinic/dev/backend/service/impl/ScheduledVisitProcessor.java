...
public class ScheduledVisitProcessor {
  ...
  public void processUpcomingVisits() {
    ...
    visitRepo.findUpcomingVisits(now, lookAhead).forEach(visit -> {
      ...
      if (!alreadyInQueue) {
        long offsetMinutes = calculateEnqueueOffsetMinutes(visit.getDoctor().getId());
        Instant enqueueTime = visit.getScheduledTime().minus(offsetMinutes, ChronoUnit.MINUTES);
  ...
  private long calculateEnqueueOffsetMinutes(Long doctorId) {
    List<Visit> historicalVisits = visitRepo.findCompletedVisitsByDoctor(doctorId);
